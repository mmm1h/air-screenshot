...
std::optional<D2D1_ROUNDED_RECT> settings_focus_bounds(
    ...
    switch (target.kind) {
        ...
        case SettingsFocusKind::global_ocr_enabled:
            rect = D2D1::RectF(258.0f, 192.0f, 886.0f, 240.0f);
            break;
        case SettingsFocusKind::shortcut: {
            ...
            if (target.index < 4) {
                const float top = 240.0f + target.index * 44.0f;
                rect = D2D1::RectF(684.0f, top + 4.0f, 870.0f, top + 40.0f);
            } else {
                const int tool_index = target.index - 4;
                const int column = tool_index / 6;
                const int row = tool_index % 6;
                const float left = column == 0 ? 420.0f : 736.0f;
                const float top = 452.0f + row * 33.0f;
                rect = D2D1::RectF(left, top + 2.0f, left + 134.0f, top + 31.0f);
            }
            break;
        }
        ...
        case SettingsFocusKind::theme: {
            ...
            rect = D2D1::RectF(
                static_cast<float>(left), 373.0f, static_cast<float>(right), 411.0f);
            break;
        }
...
void draw_shortcuts_page(SettingsState* state) {
    ...
    draw_row_group(state, 192.0f, 416.0f);
    const bool global_enabled = state->config.ocr_enabled;
    const bool global_hovered =
        global_enabled &&
        point_in_rect(state->mouse_pos, 256.0f, 192.0f, 888.0f, 240.0f);
    draw_setting_row(
        state,
        192.0f,
        240.0f,
    ...
    for (int index = 0; index < 4; ++index) {
        const float top = 240.0f + index * 44.0f;
        const std::wstring* value = get_shortcut_ptr(state->config, index);
        state->render_target->DrawTextW(
            labels[static_cast<std::size_t>(index)].data(),
            static_cast<UINT32>(labels[static_cast<std::size_t>(index)].size()),
            state->text_format.Get(),
            D2D1::RectF(276.0f, top, 628.0f, top + 44.0f),
            state->text_white_brush.Get());
        draw_hotkey_box(
            state,
            684,
            static_cast<int>(top + 4.0f),
            870,
            static_cast<int>(top + 40.0f),
            value ? value->c_str() : L"",
        ...
        if (index < 3) {
            state->render_target->DrawLine(
                D2D1::Point2F(276.0f, top + 44.0f),
                D2D1::Point2F(868.0f, top + 44.0f),
                state->separator_brush.Get(),
                1.0f);
        }
    }

    draw_section_title(state, L"标注工具", 424.0f);
    const auto tools_group = D2D1::RoundedRect(
        D2D1::RectF(256.0f, 450.0f, 888.0f, 652.0f), 10.0f, 10.0f);
    state->render_target->FillRoundedRectangle(
        tools_group, state->card_bg_brush.Get());
    state->render_target->DrawRoundedRectangle(
        tools_group, state->card_border_brush.Get(), 1.0f);
    for (int index = 4; index < shortcut_count; ++index) {
        const int tool_index = index - 4;
        const int column = tool_index / 6;
        const int row = tool_index % 6;
        ...
        state->render_target->DrawTextW(
            ...
            static_cast<UINT32>(label.size()),
            state->utility_format.Get(),
            D2D1::RectF(column_left, top, column_left + 140.0f, top + 33.0f),
            state->text_white_brush.Get());
        const int field_left = column == 0 ? 420 : 736;
        draw_hotkey_box(
            state,
            field_left,
            static_cast<int>(top + 2.0f),
            field_left + 134,
            static_cast<int>(top + 31.0f),
            value ? value->c_str() : L"",
            state->capturing_idx_ == index,
            point_in_rect(
                state->mouse_pos,
...
void draw_app_page(SettingsState* state) {
    ...
    draw_row_group(state, 152.0f, 320.0f);
    ...
    const bool tray_enabled = state->config.shell_enabled;
    const bool tray_hovered =
        tray_enabled &&
    ...
    draw_setting_row(
        ...
        264.0f,
        L"显示系统托盘图标",
        tray_enabled ? L"隐藏后后台服务与快捷键仍会继续运行。"
                     : L"启用后台运行后可设置。",
        tray_hovered,
        tray_enabled);
    draw_switch(
    ...
    const bool startup_hovered =
        startup_enabled &&
        point_in_rect(state->mouse_pos, 256.0f, 264.0f, 888.0f, 320.0f);
    draw_setting_row(
        state,
        264.0f,
        320.0f,
    ...
    draw_section_title(state, L"外观", 338.0f);
    draw_row_group(state, 364.0f, 420.0f);
    draw_setting_row(
        state,
        364.0f,
        420.0f,
    ...
    draw_section_title(state, L"应用图标", 438.0f);
    ...
    for (int index = 0; index < static_cast<int>(icon_choices.size()); ++index) {
        ...
        if (selected) {
            const D2D1_POINT_2F center = D2D1::Point2F(
                static_cast<float>(choice.right - 14), 492.0f);
            state->render_target->FillEllipse(
    ...
    state->render_target->DrawTextW(
        ...
        state->small_format.Get(),
        D2D1::RectF(276.0f, 578.0f, 868.0f, 608.0f),
        state->text_grey_brush.Get());
}
...
std::optional<SettingsFocusTarget> hit_test_settings(
    ...
    if (active_tab == 0) {
        ...
        for (int index = 0; index < 4; ++index) {
            const float top = 240.0f + index * 44.0f;
            if (point_in_rect(point, 684.0f, top + 4.0f, 870.0f, top + 40.0f)) {
        ...
        for (int index = 4; index < shortcut_count; ++index) {
            ...
            const float top = 452.0f + row * 33.0f;
            if (point_in_rect(
                    point, left, top + 2.0f, left + 134.0f, top + 31.0f)) {
                return SettingsFocusTarget{SettingsFocusKind::shortcut, index};
            }
        }
    } else if (active_tab == 4) {
        ...
        if (point_in_rect(point, 258.0f, 208.0f, 886.0f, 264.0f)) {
            return available({SettingsFocusKind::app_tray});
        }
        if (point_in_rect(point, 258.0f, 264.0f, 886.0f, 320.0f)) {
